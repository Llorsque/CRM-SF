...
(function(){
  function create(el, {options=[], onChange}){
    ...
    function renderList(filter=''){
      list.innerHTML='';
      const items=options.filter(v=>String(v).toLowerCase().includes(filter.toLowerCase()));
      items.forEach(v=>{
    ...
    function updateToggle(){ toggle.textContent = selected.size? `${selected.size} geselecteerd` : 'Alles'; }
